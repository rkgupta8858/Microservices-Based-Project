...
//	private OrderProducer orderProducer;
	...
	public OrderResponse createOrder(OrderRequest request) {
		...
		for (OrderItemRequest itemReq : request.getItems()) {
			...
			if (!stock.isInStock()) {
				throw new ProductOutOfStockException("Product out of stock: " + itemReq.getProductId());
			}
...
//		for (OrderItemRequest itemReq : request.getItems()) {
//
//			// 🔹 MOCK INVENTORY CHECK (Always true for testing)
//			boolean inStock = true;   // <---- No external call
//
//			if (!inStock) {
...
//		Order savedOrder = orderRepository.save(order);
//
//		// 🔹 MOCK PAYMENT (Always SUCCESS for testing)
//		String paymentStatus = "SUCCESS";   // <---- No external call
//
